...
public class AttachmentPanelComponent
  ...
  public AttachmentPanelComponent(Task task) {
    ...
    appendChild(h(5).textContent("ATTACHMENTS")
                    .add(badge.addClickListener(evt -> {
  ...
  public void update() {
    if (nonNull(task.getAttachments())) {
      ...
      attachmentsCountIcon.toggleDisplay(task.getAttachments()
                                             .size() > 0);
      task.getAttachments()
          .forEach(attachment -> attachmentsItemsFlexItem.appendChild(Chip.create(attachment)
                                                                          ...
                                                                          .addRemoveHandler(() -> {
                                                                            task.getAttachments()
                                                                                .remove(attachment);
                                                                            badge.setText(task.getAttachments()
                                                                                              .size() + " Files");
                                                                            attachmentsCountIcon.toggleDisplay(task.getAttachments()
                                                                                                                   .size() > 0);
                                                                            listContainer.toggleDisplay(task.getAttachments()
                                                                                                            .size() > 0);
                                                                          })));
